...
class ThreadScreen extends React.Component {
  constructor(props) {
    ...
    this.socket.on('maintenanceGroupChatResponse', (historyRes) => {
      console.log('maintenanceGroupChatResponse', JSON.stringify(historyRes));
      var parsedData = [];
      var history={};
    ...
    this.socket.on('maintenanceGroupMessageRecieved', (message) => {
      ...
      console.log('maintenanceGroupMessageRecieved', message);
      var data = [{
        text: message.msg,
  ...
  onSend(messages = []) {
    ...
    if (messages[0].image) {
      ...
      console.log('maintenanceGroupMessageSent', JSON.stringify(messages) + JSON.stringify(objMessage));
      this.socket.emit('maintenanceGroupMessageSent', objMessage)
    }
  }
